...
impl FamilyWallet {
    ...
    /// Add a new family member with specified role and spending limit.
    ///
    /// # Authorization
    /// Only Owner or Admin can add members.
    ///
    /// # Arguments
    /// * `admin` - The address performing the addition (must be Owner or Admin)
    /// * `member_address` - The address to add as a family member
    /// * `role` - The role to assign (cannot be Owner)
    /// * `spending_limit` - Per-transaction spending limit in stroops (>= 0)
    ///
    /// # Returns
    /// `Result<bool, Error>` - Ok(true) on success, Err on validation failure
    ///
    /// # Security
    /// - Validates caller is Owner or Admin
    /// - Prevents adding duplicate members
    /// - Rejects Owner role assignments
    /// - Enforces non-negative spending limits
    pub fn add_member(
        env: Env,
        admin: Address,
