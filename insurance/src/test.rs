...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // 14. set_external_ref — Comprehensive Mutation Tests
    // -----------------------------------------------------------------------
    // This section provides exhaustive external_ref mutation coverage including
    // event emission, state transitions, and security validation.

    /// Test that setting external_ref emits ExternalRefUpdatedEvent with correct data
    #[test]
    fn test_set_external_ref_emits_event_with_correct_data() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        env.ledger().set_timestamp(1_000_000u64);
        let id = client.create_policy(
        ...
        // Verify event was emitted by checking policy state
        let policy = client.get_policy(&id);
        assert!(policy.external_ref.is_some());
        assert_eq!(policy.external_ref.as_ref().unwrap(), &new_ref);
    }

    /// Test state transition: None → Some
    #[test]
    fn test_set_external_ref_none_to_some_transition() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Policy without ref"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let policy_before = client.get_policy(&id);
        assert!(policy_before.external_ref.is_none());
        
        let ref_value = String::from_str(&env, "EXT-REF-123");
        client.set_external_ref(&owner, &id, &Some(ref_value.clone()));
        
        let policy_after = client.get_policy(&id);
        assert!(policy_after.external_ref.is_some());
        assert_eq!(policy_after.external_ref.as_ref().unwrap(), &ref_value);
    }

    /// Test state transition: Some → Some (updating)
    #[test]
    fn test_set_external_ref_some_to_some_transition() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let initial_ref = String::from_str(&env, "INITIAL-REF");
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Policy with ref"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
        ...
        let policy_before = client.get_policy(&id);
        assert_eq!(policy_before.external_ref.as_ref().unwrap(), &initial_ref);
        
        let updated_ref = String::from_str(&env, "UPDATED-REF-456");
        client.set_external_ref(&owner, &id, &Some(updated_ref.clone()));
        
        let policy_after = client.get_policy(&id);
        assert_eq!(policy_after.external_ref.as_ref().unwrap(), &updated_ref);
        assert_ne!(policy_after.external_ref.as_ref().unwrap(), &initial_ref);
    }

    /// Test state transition: Some → None (clearing)
    #[test]
    fn test_set_external_ref_some_to_none_transition() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let initial_ref = String::from_str(&env, "CLEAR-ME");
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Policy to clear"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &Some(initial_ref),
        ...
        let policy_before = client.get_policy(&id);
        assert!(policy_before.external_ref.is_some());
        
        client.set_external_ref(&owner, &id, &None);
        
        let policy_after = client.get_policy(&id);
        assert!(policy_after.external_ref.is_none());
    }

    /// Test idempotent transitions
    #[test]
    fn test_set_external_ref_idempotent_same_value() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Idempotent Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let ref_value = String::from_str(&env, "SAME-REF");
        
        client.set_external_ref(&owner, &id, &Some(ref_value.clone()));
        let policy1 = client.get_policy(&id);
        assert_eq!(policy1.external_ref.as_ref().unwrap(), &ref_value);
        
        client.set_external_ref(&owner, &id, &Some(ref_value.clone()));
        let policy2 = client.get_policy(&id);
        assert_eq!(policy2.external_ref.as_ref().unwrap(), &ref_value);
    }

    /// Test sequential mutations
    #[test]
    fn test_set_external_ref_sequential_mutations() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Sequential Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let ref1 = String::from_str(&env, "REF-1");
        client.set_external_ref(&owner, &id, &Some(ref1.clone()));
        let policy1 = client.get_policy(&id);
        assert!(policy1.external_ref.is_some());
        
        let ref2 = String::from_str(&env, "REF-2");
        client.set_external_ref(&owner, &id, &Some(ref2.clone()));
        let policy2 = client.get_policy(&id);
        assert_eq!(policy2.external_ref.as_ref().unwrap(), &ref2);
        assert_ne!(policy2.external_ref.as_ref().unwrap(), &ref1);
        
        client.set_external_ref(&owner, &id, &None);
        let policy3 = client.get_policy(&id);
        assert!(policy3.external_ref.is_none());
        
        let ref3 = String::from_str(&env, "REF-3");
        client.set_external_ref(&owner, &id, &Some(ref3.clone()));
        let policy4 = client.get_policy(&id);
        assert_eq!(policy4.external_ref.as_ref().unwrap(), &ref3);
    }

    /// Test external_ref persistence across premium payments
    #[test]
    fn test_set_external_ref_persists_with_premium_payments() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Premium Payment Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let ext_ref = String::from_str(&env, "EXT-PERSIST");
        client.set_external_ref(&owner, &id, &Some(ext_ref.clone()));
        
        let policy1 = client.get_policy(&id);
        assert_eq!(policy1.external_ref.as_ref().unwrap(), &ext_ref);
        
        client.pay_premium(&caller, &id, &5_000_000i128);
        
        let policy2 = client.get_policy(&id);
        assert_eq!(policy2.external_ref.as_ref().unwrap(), &ext_ref);
    }

    /// Test minimum length (1 character)
    #[test]
    fn test_set_external_ref_minimum_length() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Min Length Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let min_ref = String::from_str(&env, "X");
        client.set_external_ref(&owner, &id, &Some(min_ref.clone()));
        let policy = client.get_policy(&id);
        assert_eq!(policy.external_ref.as_ref().unwrap(), &min_ref);
    }

    /// Test maximum length (128 characters)
    #[test]
    fn test_set_external_ref_maximum_length() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Max Length Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
    ...
    #[test]
    #[should_panic(expected = "external_ref length out of range")]
    fn test_set_external_ref_empty_string_panics() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Empty Ref Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        client.set_external_ref(&owner, &id, &Some(empty_ref));
    }

    /// Test authorization: non-owner cannot update
    #[test]
    #[should_panic(expected = "unauthorized")]
    fn test_set_external_ref_non_owner_unauthorized() {
        let (env, client, _owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Owner-Only Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let malicious_ref = String::from_str(&env, "HACKED");
        client.set_external_ref(&attacker, &id, &Some(malicious_ref));
    }

    /// Test policy creator cannot update (only owner)
    #[test]
    #[should_panic(expected = "unauthorized")]
    fn test_set_external_ref_policy_creator_cannot_update() {
        let (env, client, _owner) = setup();
        let creator = Address::generate(&env);
        let id = client.create_policy(
            &creator,
            &String::from_str(&env, "Creator Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let new_ref = String::from_str(&env, "CREATOR-REF");
        client.set_external_ref(&creator, &id, &Some(new_ref));
    }

    /// Test policy not found
    #[test]
    #[should_panic(expected = "policy not found")]
    fn test_set_external_ref_nonexistent_policy_panics() {
        let (_env, client, owner) = setup();
        let new_ref = String::from_str(&_env, "REF-FOR-GHOST");
        client.set_external_ref(&owner, &9999u32, &Some(new_ref));
    }

    /// Test special characters
    #[test]
    fn test_set_external_ref_special_characters() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Special Chars Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let special_ref = String::from_str(&env, "REF@2024-00:11:22.TX#001");
        client.set_external_ref(&owner, &id, &Some(special_ref.clone()));
        let policy = client.get_policy(&id);
        assert_eq!(policy.external_ref.as_ref().unwrap(), &special_ref);
    }

    /// Test numeric reference
    #[test]
    fn test_set_external_ref_numeric_reference() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Numeric Ref Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        let numeric_ref = String::from_str(&env, "20240328120000001");
        client.set_external_ref(&owner, &id, &Some(numeric_ref.clone()));
        let policy = client.get_policy(&id);
        assert_eq!(policy.external_ref.as_ref().unwrap(), &numeric_ref);
    }

    /// Test multiple policies isolation
    #[test]
    fn test_set_external_ref_multiple_policies_isolation() {
        let (env, client, owner) = setup();
        let caller = Address::generate(&env);
        ...
        let ref1 = String::from_str(&env, "REF-POLICY-1");
        let ref2 = String::from_str(&env, "REF-POLICY-2");
        
        client.set_external_ref(&owner, &id1, &Some(ref1.clone()));
        client.set_external_ref(&owner, &id2, &Some(ref2.clone()));
        
        let policy1 = client.get_policy(&id1);
        let policy2 = client.get_policy(&id2);
        
        assert_eq!(policy1.external_ref.as_ref().unwrap(), &ref1);
        assert_eq!(policy2.external_ref.as_ref().unwrap(), &ref2);
    }

    /// Test clearing preserves other fields
    #[test]
    fn test_set_external_ref_clearing_preserves_other_fields() {
        let (env, client, _owner) = setup();
        let caller = Address::generate(&env);
        env.ledger().set_timestamp(1_000_000u64);
        ...
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Preserve Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &Some(String::from_str(&env, "INITIAL")),
        );
        
        let policy_before = client.get_policy(&id);
        let before_premium = policy_before.monthly_premium;
        let before_coverage = policy_before.coverage_amount;
        let before_active = policy_before.active;
        
        client.set_external_ref(&_owner, &id, &None);
        
        let policy_after = client.get_policy(&id);
        assert_eq!(policy_after.monthly_premium, before_premium);
        assert_eq!(policy_after.coverage_amount, before_coverage);
        assert_eq!(policy_after.active, before_active);
        assert!(policy_after.external_ref.is_none());
    }

    /// Test that updating external_ref on deactivated policy still works
    #[test]
    fn test_set_external_ref_on_deactivated_policy_succeeds() {
        let (env, client, _owner) = setup();
        let caller = Address::generate(&env);
        let id = client.create_policy(
            &caller,
            &String::from_str(&env, "Deactivatable Policy"),
            &CoverageType::Health,
            &5_000_000i128,
            &50_000_000i128,
            &None,
        ...
        client.deactivate_policy(&_owner, &id);
        
        let new_ref = String::from_str(&env, "DEACTIVATED-MANAGED");
        client.set_external_ref(&_owner, &id, &Some(new_ref.clone()));
